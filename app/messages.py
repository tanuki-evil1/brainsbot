...
class StatusMessages(StrEnum):
    ...
    SUBSCRIPTION_NO_END_DATE = "Нет подписки"
    SUBSCRIPTION_ALREADY_ACTIVE = "У вас уже есть активная подписка"
    SUBSCRIPTION_NO_KEY = "Нет ключа"
...
Вы можете зарабатывать деньги, приглашая друзей в `Brains Secure`\\.

💰 *Как работает реферальная программа?*

1️⃣ Пригласите друга в бот
2️⃣ Друг регистрируется через ваш реферальный код
3️⃣ Вы получаете `10%` от суммы пожертвования друга"""
